...
def countries_above_threshold(threshold: int):
    """Return list of countries with Active cases above threshold."""
    return covid[covid["Active"] > threshold]["Country"].unique()


def avg_death_confirmed_above_threshold(threshold: int) -> float:
    """Return death/confirmed ratio for countries with Active > threshold."""
    filtered = covid[covid["Active"] > threshold]
    confirmed_sum = filtered["Confirmed"].sum()
    if confirmed_sum == 0:
        return 0.0
    return filtered["Deaths"].sum() / confirmed_sum


def report(threshold: int):
    """Print list of countries and ratio for one threshold."""
    countries = countries_above_threshold(threshold)
    ratio = avg_death_confirmed_above_threshold(threshold)
    print(f"List of countries with more than {threshold} active cases:", countries)
    print(
        f"Average death/confirmed for countries with more than {threshold} active cases:",
        ratio,
    )


# Run for the three thresholds
